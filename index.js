...
client.on('guildMemberAdd', async member => {
    try {
        if (member.user.bot) {
            ...
            await member.roles.add(await member.guild.roles.cache.find(role => role.name === "Bots"), `Auto-assigned with ${client.user.username} upon join`);
        } else {
            console.log('A human has joined the server.');
            await member.roles.add(await member.guild.roles.cache.find(role => role.name === "Humans"), `Auto-assigned with ${client.user.username} upon join`);
        }
    } catch (error) {
        console.error(error);
    }
        ...
        if (PHOTOGRAPHY_CHANNEL !== undefined && message.channel.name === PHOTOGRAPHY_CHANNEL) { // Photography is enabled and a message was sent to the channel
            if (message.attachments.size > 0) {
                ...
                if (utils.isImageAttachment(firstAttachment)) {
                    message.startThread({
                        name: messageContent.length > 0 ? messageContent : message.author.username + '\'s Photo',
                        reason: `Auto-generated by ${client.user.username} upon attachment upload`
                    }).then(thread => {
                        let camera;
                        utils.saveImage(firstAttachment).then(imagePath => {
                            // TODO: Extract camera metadata
                            camera = utils.getCameraFromImage(imagePath);
                            utils.deleteFile(imagePath); // Clean the temp folder
                        }).catch(() => {
                            camera = 'Unknown';
                        }).finally(async () => {
                            await thread.send(utils.generatePhotographyEmbed(firstAttachment.name, firstAttachment.url, camera, firstAttachment.width, firstAttachment.height));
                        });
                    });
                }
            }
